...
public class RevealDrawable extends Drawable implements Animatable {
	...
	private RadialGradient getShader(ColorChangeTask task) {
		if (mShader == null) {
			if (task.isOut) {
				...
				mShader = new RadialGradient(task.x, task.y, GRADIENT_RADIUS,
						new int[] { 0, color_middle, mCurColor }, GRADIENT_STOPS,
						Shader.TileMode.CLAMP);
			} else {
				int color_middle = ColorUtil.getColor(task.color, 0f);
				mShader = new RadialGradient(task.x, task.y, GRADIENT_RADIUS,
						new int[] { 0, color_middle, task.color }, GRADIENT_STOPS,
						Shader.TileMode.CLAMP);
			}
		}
	...
	private void fillCanvasWithHole(Canvas canvas, ColorChangeTask task,
			float radius, boolean transparent) {
		if (transparent)
			return;
	...
	private void fillCircle(Canvas canvas, float x, float y, float radius,
		...
		if (!isRunning())
			...
			if (mAnimProgress == 0f)
				...
				float radius = mMaxRadius
						* task.interpolator.getInterpolation(mAnimProgress);
				...
				fillCircle(canvas, task.x, task.y, radius, task.color,
						mNextColorTransparent);
			} else {
				float radius = mMaxRadius
						* task.interpolator.getInterpolation(mAnimProgress);
				...
				fillCircle(canvas, task.x, task.y, radius, mCurColor,
						mCurColorTransparent);
			}
		}
	}
	...
	public void changeColor(ColorChangeTask... tasks) {
		synchronized (RevealDrawable.class) {
			if (!isRunning()) {
				...
				ColorChangeTask[] newTasks = new ColorChangeTask[curLength
						+ tasks.length];
	...
	private void resetAnimation() {
		...
		mMaxRadius = getMaxRadius(mTasks[mCurTask].x, mTasks[mCurTask].y,
				getBounds());
		mShader = null;
	}
	@Override
	...
	private void update() {
		...
		synchronized (RevealDrawable.class) {
			mAnimProgress = Math.min(1f, (float) (curTime - mStartTime)
					/ mTasks[mCurTask].duration);
			if (mAnimProgress == 1f) {
	...
	public static class ColorChangeTask {
		...
		public ColorChangeTask(int color, int duration, Interpolator interpolator,
				float x, float y, boolean out) {
			this.color = color;
			this.duration = duration;
			this.interpolator = interpolator == null ? new DecelerateInterpolator()
					: interpolator;
			this.x = x;
			this.y = y;
			this.isOut = out;
