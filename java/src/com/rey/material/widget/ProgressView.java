...
public class ProgressView extends View {
	...
	private void applyStyle(Context context, AttributeSet attrs,
		...
		a.recycle();
		if (mProgressId == 0)
			mProgressId = mCircular ? R.style.Material_Drawable_CircularProgress
					: R.style.Material_Drawable_LinearProgress;
		if (mCircular)
			mProgressDrawable = new CircularProgressDrawable.Builder(context,
					mProgressId).build();
		else
			mProgressDrawable = new LinearProgressDrawable.Builder(context,
					mProgressId).build();
	...
	public float getSecondaryProgress() {
		if (mCircular)
			return ((CircularProgressDrawable) mProgressDrawable)
					.getSecondaryProgress();
		else
			return ((LinearProgressDrawable) mProgressDrawable)
					.getSecondaryProgress();
	}
	...
	public void setSecondaryProgress(float percent) {
		if (mCircular)
			...
			((LinearProgressDrawable) mProgressDrawable)
					.setSecondaryProgress(percent);
	}
