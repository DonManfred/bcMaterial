...
public class DatePicker extends ListView implements AbsListView.OnScrollListener{
    ...
    public int getAnimDuration() {
		...
		public void setSelectionRadius(float SelectionRadius) {
    ...
    private void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        TextSize = a.getDimensionPixelSize(R.styleable.DatePicker_dp_dayTextSize, context.getResources().getDimensionPixelOffset(R.dimen.abc_text_size_caption_material));
        TextColor = a.getColor(R.styleable.DatePicker_dp_textColor, 0xFF000000);
        TextHighlightColor = a.getColor(R.styleable.DatePicker_dp_textHighlightColor, 0xFFFFFFFF);
        TextLabelColor = a.getColor(R.styleable.DatePicker_dp_textLabelColor, 0xFF767676);
        TextDisableColor = a.getColor(R.styleable.DatePicker_dp_textDisableColor, 0xFF767676);
        SelectionColor = a.getColor(R.styleable.DatePicker_dp_selectionColor, ThemeUtil.colorPrimary(context, 0xFF000000));
        AnimDuration = a.getInteger(R.styleable.DatePicker_dp_animDuration, context.getResources().getInteger(android.R.integer.config_mediumAnimTime));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        SelectionRadius = Math.min(mDayWidth, mDayHeight) / 2f;
    }
    ...
    private class MonthView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if(mSelectedDay > 0){
                ...
                float radius = mRunning ? mInInterpolator.getInterpolation(mAnimProgress) * SelectionRadius : SelectionRadius;
            ...
            if(mRunning && mPreviousSelectedDay > 0){
                ...
                x = (col + 0.5f) * mDayWidth + paddingLeft;
                y = (row + 0.5f) * mDayHeight + paddingTop;
                float radius = (1f - mOutInterpolator.getInterpolation(mAnimProgress)) * SelectionRadius;
                mPaint.setColor(SelectionColor);
                canvas.drawCircle(x, y, radius, mPaint);
            }
            ...
            mPaint.setColor(TextLabelColor);
            ...
            for(int day = 1; day <= mMaxDay; day++){
                if(day == mSelectedDay)
                    mPaint.setColor(TextHighlightColor);
                else if(day < mMinAvailDay || day > maxDay)
                    mPaint.setColor(TextDisableColor);
                else if(day == mToday)
                    mPaint.setColor(SelectionColor);
                else
                    mPaint.setColor(TextColor);
        ...
        private void update(){
            ...
            mAnimProgress = Math.min(1f, (float)(curTime - mStartTime) / AnimDuration);
