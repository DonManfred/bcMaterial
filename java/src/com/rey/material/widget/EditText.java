...
public class EditText extends FrameLayout {
	...
	private void applyStyle(Context context, AttributeSet attrs,
		...
		mSupportMode = a.getInteger(R.styleable.EditText_et_supportMode,
				SUPPORT_MODE_NONE);
		mAutoCompleteMode = a.getInteger(R.styleable.EditText_et_autoCompleteMode,
		...
		int dividerAnimDuration = a.getInteger(
				R.styleable.EditText_et_dividerAnimDuration, context.getResources()
		...
		if (mLabelEnable) {
			...
			int labelEllipsize = a.getInteger(R.styleable.EditText_et_labelEllipsize,
					0);
			mLabelInAnimId = a.getResourceId(R.styleable.EditText_et_labelInAnim, 0);
			mLabelOutAnimId = a
					.getResourceId(R.styleable.EditText_et_labelOutAnim, 0);
		...
		if (mSupportMode != SUPPORT_MODE_NONE) {
			...
			case SUPPORT_MODE_HELPER_WITH_ERROR:
				...
				mSupportHelper = ThemeUtil.getString(a, R.styleable.EditText_et_helper,
						supportHelper);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int tempHeightSpec = MeasureSpec
		...
		switch (heightMode) {
		case MeasureSpec.UNSPECIFIED:
			height = labelHeight + inputHeight + supportHeight + getPaddingTop()
					+ getPaddingBottom();
			break;
		case MeasureSpec.AT_MOST:
			height = Math.min(heightSize, labelHeight + inputHeight + supportHeight
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mLabelView != null) {
			mLabelView.layout(childLeft, childTop, childRight,
	...
	public void setError(CharSequence error) {
		...
		if (mSupportError != null) {
			...
			mSupportView.setText(mSupportMode == SUPPORT_MODE_HELPER ? mSupportError
					: TextUtils.concat(mSupportHelper, ", ", mSupportError));
		} else {
			mSupportView.setTextColor(mSupportColors);
	 ...
	 * @param keyCode
	 *          the last character inserted in the edit box; beware that this will
	 *          be null when text is being added through a soft input method.
	 */
	protected void performFiltering(CharSequence text, int keyCode) {
		switch (mAutoCompleteMode) {
		case AUTOCOMPLETE_MODE_SINGLE:
			((InternalAutoCompleteTextView) mInputView).superPerformFiltering(text,
					keyCode);
			break;
		case AUTOCOMPLETE_MODE_MULTI:
			((InternalMultiAutoCompleteTextView) mInputView).superPerformFiltering(
					text, keyCode);
			break;
		}
	}
	 ...
	 * <code>null</code> if {@link #setAdapter} was not called with a Filterable.
	 */
	...
	/**
	 * <p>
	 * Returns the listener that is notified whenever the user selects an item in
	 * the drop down list.
	 * </p>
	 * <p>
	 * Only work when autoCompleMode is AUTOCOMPLETE_MODE_SINGLE or
	 ...
	 * adapter. Notably, when the AutoCompleteTextView is closed or released, the
	 * adapter is not notified. A common case is the use of
	 ...
	 *
	 * @param validator
	 *          The validator used to validate the text entered in this widget.
	 *
	 * @see #getValidator()
	...
	/**
	 * Returns the Validator set with {@link #setValidator}, or <code>null</code>
	 * if it was not set.
	 * <p>
	 * Only work when autoCompleMode is AUTOCOMPLETE_MODE_SINGLE or
	 * AUTOCOMPLETE_MODE_MULTI
	 ...
	 * {@link #setText} if there are any text changed listeners forces the buffer
	 * type to Editable if it would not otherwise be and does call this method.
	 */
	...
	/**
	 * @return the current key listener for this TextView. This will frequently be
	...
	public int getTotalPaddingBottom() {
		return getPaddingBottom() + mInputView.getTotalPaddingBottom()
				+ (mSupportMode != SUPPORT_MODE_NONE ? mSupportView.getHeight() : 0);
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalMultiAutoCompleteTextView) mInputView).superOnKeyDown(
					keyCode, event);
	}
	...
	public boolean onKeyMultiple(int keyCode, int repeatCount, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalAutoCompleteTextView) mInputView).superOnKeyMultiple(
	...
	public boolean onKeyPreIme(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalMultiAutoCompleteTextView) mInputView).superOnKeyPreIme(
					keyCode, event);
	}
	...
	public boolean onKeyShortcut(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalAutoCompleteTextView) mInputView).superOnKeyShortcut(
					keyCode, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalAutoCompleteTextView) mInputView).superOnKeyUp(keyCode,
					event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView).superOnKeyUp(
					keyCode, event);
	}
	...
	protected void onSelectionChanged(int selStart, int selEnd) {
		...
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			((InternalAutoCompleteTextView) mInputView).superOnSelectionChanged(
					selStart, selEnd);
		else
			((InternalMultiAutoCompleteTextView) mInputView).superOnSelectionChanged(
					selStart, selEnd);
		...
		if (mDividerCompoundPadding) {
			...
			if (mSupportMode != SUPPORT_MODE_NONE)
				mSupportView.setPadding(mDivider.getPaddingLeft(),
						mSupportView.getPaddingTop(), mDivider.getPaddingRight(),
	...
	public void setCompoundDrawablesRelativeWithIntrinsicBounds(Drawable start,
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
			mInputView.setCompoundDrawablesRelativeWithIntrinsicBounds(start, top,
					end, bottom);
		else
	 ...
	 * {@link android.R.id#copy} or {@link android.R.id#paste} ids as parameters.
	 *
	 ...
	 * .
	 *
	 * Note that text selection mode is not started when a TextView receives focus
	 * and the {@link android.R.attr#selectAllOnFocus} flag has been set. The
	 * content is highlighted in that case, to allow for quick replacement.
	 */
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	 ...
	 * variants that have not been compacted to fit Latin-based vertical metrics,
	 * and also increases top and bottom bounds to provide more space.
	 *
	 * @param elegant
	...
	/**
	 * Sets the text to be displayed when the text of the TextView is empty, from
	 * a resource.
	 *
	 * @attr ref android.R.styleable#TextView_hint
	 */
	 ...
	 * Bundle that will be filled in when creating an input connection. The given
	 * integer is the resource ID of an XML resource holding an
	 ...
	 * {@link #setFocusable} again after calling this to get the focusability back
	 * the way you want it.
	 *
	 * @attr ref android.R.styleable#TextView_numeric
	 * @attr ref android.R.styleable#TextView_digits
	 ...
	 * This can be null to disallow using the arrow keys to move the cursor or
	 * scroll the view.
	 * <p>
	 * Be warned that if you want a TextView with a key listener or movement
	 * method not to be focusable, or if you want a TextView without a key
	 ...
	 * supplied to the IME is selected by the user. Setting this means that the
	 * normal hard key event will not insert a newline into the text view, even if
	 * it is multi-line; holding down the ALT modifier will, however, allow the
	 * user to insert a newline character.
	 */
	 ...
	 * view. Key presses in software input methods will generally not trigger the
	 ...
	 *
	 * However, this is like this for legacy reasons and we cannot break existing
	 * apps. This method is useful since it matches what the user can see
	 * (obfuscated text or not).
	 *
