...
public class Slider extends View {
	...
	public void setDiscreteMode(boolean DiscreteMode) {
		this.mDiscreteMode = DiscreteMode;
	}
	...
	public interface OnPositionChangeListener {
		public void onPositionChanged(Slider view, float oldPos, float newPos,
	...
	private void applyStyle(Context context, AttributeSet attrs,
		...
		mDiscreteMode = a.getBoolean(R.styleable.Slider_sl_discreteMode,
				mDiscreteMode);
		mPrimaryColor = a.getColor(R.styleable.Slider_sl_primaryColor,
				ThemeUtil.colorControlActivated(context, 0xFF000000));
		mSecondaryColor = a.getColor(R.styleable.Slider_sl_secondaryColor,
				ThemeUtil.colorControlNormal(context, 0xFF000000));
		mTrackSize = a.getDimensionPixelSize(R.styleable.Slider_sl_trackSize,
				ThemeUtil.dpToPx(context, 2));
		...
		mThumbBorderSize = a.getDimensionPixelSize(
				R.styleable.Slider_sl_thumbBorderSize, ThemeUtil.dpToPx(context, 2));
		mThumbRadius = a.getDimensionPixelSize(R.styleable.Slider_sl_thumbRadius,
				ThemeUtil.dpToPx(context, 10));
		mThumbFocusRadius = a.getDimensionPixelSize(
				R.styleable.Slider_sl_thumbFocusRadius, ThemeUtil.dpToPx(context, 14));
		mTravelAnimationDuration = a.getInteger(
				R.styleable.Slider_sl_travelAnimDuration, context.getResources()
						.getInteger(android.R.integer.config_mediumAnimTime));
		mTransformAnimationDuration = a.getInteger(
				R.styleable.Slider_sl_travelAnimDuration, context.getResources()
						.getInteger(android.R.integer.config_shortAnimTime));
		int resId = a.getResourceId(R.styleable.Slider_sl_interpolator, 0);
		mInterpolator = resId != 0 ? AnimationUtils
				.loadInterpolator(context, resId) : new DecelerateInterpolator();
		mGravity = a.getInt(R.styleable.Slider_android_gravity,
		...
		mTextSize = a.getDimensionPixelSize(
				R.styleable.Slider_sl_textSize,
	...
	private void measureText() {
		...
		float maxWidth = (float) (mThumbRadius * Math.sqrt(2) * 2 - ThemeUtil
				.dpToPx(getContext(), 8));
		if (width > maxWidth) {
	...
	public float getPosition() {
		return mThumbMoveAnimator.isRunning() ? mThumbMoveAnimator.getPosition()
				: mThumbPosition;
	}
	...
	private void setPosition(float pos, boolean moveAnimation,
			boolean transformAnimation) {
		...
		if (change && mOnPositionChangeListener != null)
			mOnPositionChangeListener.onPositionChanged(this, oldPos, newPos,
					oldValue, newValue);
	}
	...
	public int getSuggestedMinimumWidth() {
		return (mDiscreteMode ? (int) (mThumbRadius * Math.sqrt(2))
				: mThumbFocusRadius) * 4 + getPaddingLeft() + getPaddingRight();
	}

	@Override
	public int getSuggestedMinimumHeight() {
		return (mDiscreteMode ? (int) (mThumbRadius * (4 + Math.sqrt(2)))
				: mThumbFocusRadius * 2) + getPaddingTop() + getPaddingBottom();
	}
	...
	private boolean isThumbHit(float x, float y, float radius) {
		...
		return x >= cx - radius && x <= cx + radius && y >= cy - radius
				&& y < cy + radius;
	}
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			mIsDragging = isThumbHit(event.getX(), event.getY(), mThumbRadius)
					&& !mThumbMoveAnimator.isRunning();
			mMemoPoint.set(event.getX(), event.getY());
			if (mIsDragging)
				mThumbRadiusAnimator.startAnimation(mDiscreteMode ? 0
						: mThumbFocusRadius);
			break;
		case MotionEvent.ACTION_MOVE:
			if (mIsDragging) {
				if (mDiscreteMode) {
					float position = correctPosition(Math
							.min(
									1f,
									Math.max(0f,
											(event.getX() - mDrawRect.left) / mDrawRect.width())));
		...
		case MotionEvent.ACTION_UP:
			...
			} else if (distance(mMemoPoint.x, mMemoPoint.y, event.getX(),
					event.getY()) <= mTouchSlop) {
				float position = correctPosition(Math.min(1f,
						Math.max(0f, (event.getX() - mDrawRect.left) / mDrawRect.width())));
				setPosition(position, true);
			}
			break;
	...
	private void getTrackPath(float x, float y, float radius) {
		...
		if (radius - 1f < halfStroke) {
			if (mTrackCap != Paint.Cap.ROUND) {
				...
				if (x > mDrawRect.left) {
					mTempRect.set(mDrawRect.left, y - halfStroke, mDrawRect.left
							+ mTrackSize, y + halfStroke);
				...
				if (x < mDrawRect.right) {
					mTempRect.set(mDrawRect.right - mTrackSize, y - halfStroke,
							mDrawRect.right, y + halfStroke);
			...
			if (mTrackCap != Paint.Cap.ROUND) {
				mTempRect.set(x - radius + 1f, y - radius + 1f, x + radius - 1f, y
						+ radius - 1f);
				...
				if (x - radius > mDrawRect.left) {
					float angle2 = (float) (Math.acos(Math.max(0f, (mDrawRect.left
							+ halfStroke - x + radius)
							/ halfStroke))
							/ Math.PI * 180);
				...
				if (x + radius < mDrawRect.right) {
					float angle2 = (float) Math.acos(Math.max(0f, (x + radius
							- mDrawRect.right + halfStroke)
							/ halfStroke));
					mRightTrackPath.moveTo(
							(float) (mDrawRect.right - halfStroke + Math.cos(angle2)
									* halfStroke), (float) (y + Math.sin(angle2) * halfStroke));
	...
	private Path getMarkPath(Path path, float cx, float cy, float radius,
			float factor) {
		if (path == null)
			path = new Path();
		else
	...
	public void draw(@NonNull Canvas canvas) {
		...
		int filledPrimaryColor = ColorUtil.getMiddleColor(mSecondaryColor,
				isEnabled() ? mPrimaryColor : mSecondaryColor, mThumbFillPercent);
		...
		if (mDiscreteMode) {
			...
			if (factor > 0) {
				...
				canvas.drawText(getValueText(), x, y + mTextHeight / 2f - mThumbRadius
						* factor, mPaint);
				canvas.restoreToCount(saveCount);
			}

			float radius = isEnabled() ? mThumbCurrentRadius : mThumbCurrentRadius
					- mThumbBorderSize;
			if (radius > 0) {
				mPaint.setColor(filledPrimaryColor);
			...
			if (mThumbFillPercent == 1)
				...
				float strokeWidth = (radius - mThumbBorderSize) * mThumbFillPercent
						+ mThumbBorderSize;
	...
	class ThumbRadiusAnimator implements Runnable {
		...
		public void run() {
			...
			float progress = Math.min(1f, (float) (curTime - mStartTime)
					/ mTransformAnimationDuration);
	...
	class ThumbStrokeAnimator implements Runnable {
		...
		public void run() {
			...
			mThumbFillPercent = (mFillPercent - mStartFillPercent) * value
					+ mStartFillPercent;
	...
	class ThumbMoveAnimator implements Runnable {
		...
		public void resetAnimation() {
			...
			mDuration = mDiscreteMode && !mIsDragging ? mTransformAnimationDuration
					* 2 + mTravelAnimationDuration : mTravelAnimationDuration;
		}
		...
		public void run() {
			...
			if (mDiscreteMode) {
				if (mIsDragging) {
					mThumbPosition = (mPosition - mStartPosition) * value
							+ mStartPosition;
					mThumbFillPercent = (mFillPercent - mStartFillPercent) * value
							+ mStartFillPercent;
				} else {
					float p1 = (float) mTravelAnimationDuration / mDuration;
					float p2 = (float) (mTravelAnimationDuration + mTransformAnimationDuration)
							/ mDuration;
					if (progress < p1) {
				...
				if (progress < 0.2)
					mThumbCurrentRadius = Math.max(mThumbRadius + mThumbBorderSize
							* progress * 5, mThumbCurrentRadius);
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "Slider.SavedState{"
					+ Integer.toHexString(System.identityHashCode(this)) + " pos="
					+ position + "}";
		}
