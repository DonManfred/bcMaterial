...
public class ContactChipSpan extends ReplacementSpan {
	...
	public ContactChipSpan(CharSequence name, int height, int maxWidth,
			int paddingLeft, int paddingRight, Typeface typeface, int textColor,
			int textSize, int backgroundColor) {
		...
		mHeight = height;
		mWidth = Math.round(Math.min(maxWidth,
				mPaint.measureText(name, 0, name.length()) + paddingLeft + paddingRight
						+ height));

		int outerWidth = Math.max(0, mWidth - mPaddingLeft - mPaddingRight
				- mHeight);
		mMetrics = BoringLayout.isBoring(mContactName, mTextPaint, mMetrics);
		mBoringLayout = BoringLayout.make(mContactName, mTextPaint, outerWidth,
				Layout.Alignment.ALIGN_NORMAL, 1f, 1f, mMetrics, true,
				TextUtils.TruncateAt.END, outerWidth);
	}
	...
	public void setImage(Bitmap bm) {
		if (mBitmap != bm) {
			...
			if (mBitmap != null) {
				...
				mMatrix.reset();
				float scale = mHeight
						/ (float) Math.min(mBitmap.getWidth(), mBitmap.getHeight());
				mMatrix.setScale(scale, scale, 0, 0);
				mMatrix.postTranslate((mHeight - mBitmap.getWidth() * scale) / 2,
	...
	public void draw(Canvas canvas, CharSequence text, int start, int end,
		...
		if (mContactName != null && mBoringLayout != null) {
			canvas.translate(mHeight + mPaddingLeft,
					(mHeight - mBoringLayout.getHeight()) / 2f);
